...
This web page contains some of the exercises we will go through during the course.  
...
There are two pieces of softeware you should install to work with R: first, base R itself, and then R Studio, which is is an almost universally used user interface - a so called integrated development environment (IDE) - that makes working with R much easier. 

Follow the instructions in the videos below to install R and R Studio on your computer - both are available for Windows, Mac OS, and Linux.
...
However, in a virtual meeting, practical exercises are a bit of a challenge: it is more difficult to provide 1:1 support and following the course and using R at the same time might not be possible, if both application windows have to share the same screen.



Therefore, practical examples are not conducted in R itself, but on the next pages of this website. You will there find code blocks, that allow you to enter and run your own R code

It allows you to write R code in more save environment, and it provides automatic feedback and support. 
...
Now, give it a try, if you want: replace `"YOUR_NAME"` in the code block below with your own name (e.g. type `id = "Alice"`) and then click the <span style="display: inline;">
  <a class="btn btn-success btn-xs btn-tutorial-run" type="button" title="" data-icon="fa-play"><i class="fa fa-play"></i> Run Code</a> </span> button on the right. The code is evaluated and shows you the resulting output below - this is exactly the same as you would get from running the code within R.
...
#### Exervise 1.2 - Now, it's your turn!
